<template>
	<div>
		...
		<ele-pro-table  ref="table" method="POST" :needPage="false" :where="where" :datasource="url"
			row-key="ID" default-expand-all :parse-data="parseData" :columns="columns" :selection.sync="selection"
			:cell-style="{'text-align':'left'}" :header-cell-style="{'text-align':'left'}">
...
<script>
	...
	export default {
		...
		props: {
			...
			columnsshow: Boolean // show 显示公司类型 公司分类 false不显示
		},
		data() {
			return {
				// 表格数据接口
