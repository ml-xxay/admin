<template>
	<div>
      <el-table  v-if="refresh" ref="table" row-key="ID" border @cell-click="tableCellClick" default-expand-all
        :cell-class-name="
      ...
      " :data="tableData" style="width: 100%;margin-top: 0px">
        ...
        <el-table-column prop="ID" label="标准ID" width="110">
        </el-table-column>
        ...
        <el-table-column prop="GONGSIFL" label="公司分类" width="110">
        </el-table-column>

        <el-table-column prop="GONGSILX" label="公司类型" width="110">
        </el-table-column>

        <el-table-column prop="GONGNENGYF" label="功能用房" width="250">
        </el-table-column>

        <el-table-column prop="ZIXIANGYI" label="子项一" width="250">
        </el-table-column>

        <el-table-column prop="ZIXIANGER" label="子项二" width="250">
        </el-table-column>
    ...
    methods: {
      getData() {
        this.$http.post(this.serviceurl + '/TM_getallinfo_controller', {
          ...
          equalmap: JSON.stringify({"GONGSIID": this.GONGSIID,"GONGSIGUIMOID":this.GONGSIGUIMOID}),    //公司类型ID转换成  x类公司
        }).then(res => {
          this.SHENGONGSISJ = res.data.data;     //在省公司执行标准数据表中  查找 选中公司 选中房产后 的数据

          this.$http.post(this.serviceurl + '/TM_getallinfo_controller', {
            table: "TMDB_PMS_BIAOZHUN",
            equalmap: JSON.stringify({"GONGSILX": myutils.getGONGSILXNAME(this.GONGSILX)}),    //公司类型ID转换成  x类公司
