  ...
  methods: {
    ...
    save() {
      ...
      if (this.isUpdate) {
        ...
        formData.append('ZONGHEGLRS', this.form.ZONGHEGLRS)
        formData.append('SHITANGMJ', this.form.SHITANGMJ)
        formData.append('ZONGRS', this.form.ZONGRS)
        formData.append('XIAOYOULOUDONGSHU', this.form.XIAOYOULOUDONGSHU)
        formData.append('BEIZHU', this.form.BEIZHU)

        this.$requestsu.post("/TM_updateone_controller", formData).then(function (response) {
          console.log(response);
          if (response.data.code === 200) {
            self.$message('修改公司数据成功');
            self.updateVisible(false);
            self.reload()
          } else {
            self.$message('修改公司数据失败');
            self.updateVisible(false);
            self.reload()
          }
        })
        ...
        this.$refs.form.validate(async valid => { 
          ...
          this.$requestsu.post("/TM_insertone_controller", formData).then(function (response) {
            console.log(response);
            if (response.data.code === 200) {
              self.$message('添加公司数据成功');
              self.updateVisible(false);
              self.reload()
            } else {
              self.$message('添加公司数据失败');
              self.updateVisible(false);
              self.reload()
            }
          })
