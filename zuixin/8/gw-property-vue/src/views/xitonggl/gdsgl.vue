  ...
  <div class="ele-body">
    <el-card shadow="never">
      ...
      <el-form
        ...
        <el-row :gutter="15">
          <el-col :md="5" :sm="12">
            <el-form-item label="供电所名称" label-width="100px">
              <el-input
                clearable
...
<script>
import UserEdit from './gongdiansuo/gds-edit.vue';
import UserImport from './gongdiansuo/gds-import.vue';
...
export default {
  ...
  data() {
    return {
      ...
      where: {
        ...
        orderarray: JSON.stringify([{"FUJIID":"ASC","GONGSIGUIMOID":"ASC","ID":"ASC"}]),
  ...
  methods: {
    ...
    remove(row) {
      ...
      this.$requestsu.post("/TM_deletebatchbydeletekeys_controller", formData).then(function (response) {
        ...
        if (response.data.code === 200) {
          self.$message('删除供电所数据成功');
          self.reload()
        } else {
          self.$message('删除供电所数据失败');
        }
      })
    ...
    removeBatch() {
      ...
      this.$confirm('确定要删除选中的供电所数据吗?', '提示', {
        type: 'warning'
      }).then(() => {
        ...
        this.$requestsu.post("/TM_deletebatchbydeletekeys_controller", formData).then(function (response) {
          console.log(response);
          if (response.data.code === 200) {
            self.$message('批量删除供电所数据成功');
            self.reload()
          } else {
            self.$message('批量删除供电所数据失败');
          }
        })
          .catch(function (error) {
            console.log(error);
